/*
* In this program 8 images are displayed on GLCD sequentially. The speed of the animal appears to change ie. when Joystick
* is moved up the speed increases and when it is moved down the speed decreases. (Hold the joystick in the same position to
 * observe the changes.
 * ADC and GLCD are used in this program.
 * Ensure that PD0, PD1 are not connected to PB6 and PB7 on TIVA Board.
*/
